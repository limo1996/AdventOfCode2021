  ...
  for (auto line : lines) {
    if (line[0] == line[2]) {
      ...
      for (int i = min(line[0], line[2]); i <= max(line[0], line[2]); i++)
        board[i][line[1]]++;
    } else if (abs(line[0] - line[2]) == abs(line[1] - line[3])) {
      for (int i = 0; i <= abs(line[0] - line[2]); i++) {
        int x = line[0] < line[2] ? line[0] + i : line[0] - i;
        int y = line[1] < line[3] ? line[1] + i : line[1] - i;
        board[x][y]++;
      }
    } else {
      assert(false);
